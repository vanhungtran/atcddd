...
res <- list(
  ...
  ddd = data.frame(
    source_code = c("D01AA", "D01AA"),
    atc_code = c("D01AA01", "D01AA02"),
    atc_name = c("nystatin", "natamycin"),
    ddd = c(NA, NA),
    uom = c(NA, NA),
    adm_r = c(NA, NA),
    note = c(NA, NA)
...
```{r viz_levels, eval=FALSE, fig.width=8, fig.height=5}
# Calculate ATC code levels
code_levels <- res$codes %>%
  mutate(
    level = case_when(
      stringr::str_length(atc_code) == 1 ~ "Level 1",
      stringr::str_length(atc_code) == 3 ~ "Level 2",
      stringr::str_length(atc_code) == 4 ~ "Level 3",
      stringr::str_length(atc_code) == 5 ~ "Level 4",
      stringr::str_length(atc_code) == 7 ~ "Level 5",
      TRUE ~ "Other"
    )
  ) %>%
  count(level) %>%
...
1. **Slow crawling**: Increase the `rate` parameter or use `max_codes` for testing
2. **Network errors**: Check internet connection and WHO website availability
3. **Memory issues**: Crawl specific groups instead of all at once
